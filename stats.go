...
func statsFunc() {
	...
	elapsed, avgIterNextDuration, n = iterateOverRootCollection(client.Collection("folders").
		Where("ParentID", "==", SportsParent).
		Documents(ctx))
	...
	elapsed, avgIterNextDuration, n = iterateOverRootCollection(
		client.Collection("folders").
			Where("ParentID", "==", FoodParent).
			Documents(ctx))
	statistics = append(statistics, stats{
		name:        "food roots",
		n:           n,
	...
	elapsed, avgIterNextDuration, n = iterateOverSubcollection(
		client.Collection("folders/foods/folders").
